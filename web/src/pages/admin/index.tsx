...
import { Card, DataTable, Icon, PageHeader, Status } from '../../components/common/ui';
import { periods, useTimetable, weekdays, type SlotId } from '../../stores/timetableStore';
import { request } from '../../services/api/client';
import { applyDemoScan, demoAttendanceCards, demoAttendanceRecords, getAttendanceCards, getAttendanceRecords, scanAttendance, type AttendanceCard, type AttendanceOverview } from '../../services/api/attendance';
...
function formatAttendanceTime(value?: string | null) { return value ? new Date(value).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' }) : 'Not scanned'; }

export function AdminAttendance() {
  const [students, setStudents] = useState<AttendanceOverview[]>(demoAttendanceRecords); const [cards, setCards] = useState<AttendanceCard[]>(demoAttendanceCards); const [message, setMessage] = useState('Showing demo attendance data. Run migration and seed to replace this with database rows.'); const [scanning, setScanning] = useState(false);
  const load = async () => { const [records, linkedCards] = await Promise.all([getAttendanceRecords(), getAttendanceCards()]); setStudents(records.length ? records : demoAttendanceRecords); setCards(linkedCards.length ? linkedCards : demoAttendanceCards); setMessage(records.length ? '' : 'Showing demo attendance data. Run migration and seed to replace this with database rows.'); };
  useEffect(() => { void load().catch(() => setMessage('Showing demo attendance data. Backend attendance API is not ready yet.')); }, []);
  const demoScan = async (uid: string) => { setScanning(true); setMessage(''); try { const result = await scanAttendance(uid, 'Dashboard demo scanner'); setMessage(`${result.attendance.student_name}: ${result.created ? 'attendance recorded' : 'scan time updated'} for today.`); await load(); } catch { const updated = applyDemoScan(uid); setStudents([...demoAttendanceRecords]); setCards([...demoAttendanceCards]); setMessage(`${updated.student_name}: demo scan recorded locally. Run backend migration/seed for real DB scan.`); } finally { setScanning(false); } };
  const presentToday = students.filter(student => student.today_status === 'present').length;
  return <AdminLayout><PageHeader title="Attendance" subtitle="NFC attendance records, linked cards, and live classroom presence." /><div className="attendance-dashboard-grid"><Card><div className="card-title"><h2>Today</h2><Icon>fact_check</Icon></div><strong className="metric-xl">{presentToday}/{students.length}</strong><span className="muted">Students marked present by NFC</span></Card><Card><div className="card-title"><h2>Linked NFC Cards</h2><Icon>nfc</Icon></div><strong className="metric-xl">{cards.length}</strong><span className="muted">Demo cards plus real NFC cards</span></Card><Card><div className="card-title"><h2>Real scan URL</h2><Icon>link</Icon></div><p className="muted">Write this pattern to an NFC tag: /attendance/scan?card=DEMO-NFC-001</p><button className="primary-btn" disabled={scanning} onClick={() => void demoScan('DEMO-NFC-001')}><Icon>nfc</Icon>{scanning ? 'Recording...' : 'Quick demo scan'}</button></Card></div>{message && <p className="timetable-message"><Icon>info</Icon>{message}</p>}<Card><div className="card-title"><h2>Student Attendance</h2><span className="muted">Live database rows with demo history</span></div><div className="table-wrap"><table><thead><tr><th>Student</th><th>Class</th><th>Today</th><th>Attendance</th><th>Last scan</th></tr></thead><tbody>{students.length ? students.map(student => <tr key={student.student_id}><td><b>{student.student_name}</b><br /><small>{student.student_code}</small></td><td>{student.grade} • {student.section}</td><td><Status>{student.today_status === 'not_marked' ? 'Pending' : student.today_status}</Status></td><td>{student.attendance_percentage}%</td><td>{formatAttendanceTime(student.latest_scan_at)}</td></tr>) : <tr><td colSpan={5}>No student attendance records found.</td></tr>}</tbody></table></div></Card><Card><div className="card-title"><h2>NFC Card Mapping</h2><span className="muted">Demo card UID and scan action</span></div>{cards.map(card => <div className="setup-subject attendance-card-row" key={card.uid}><div><b>{card.uid}</b><small>{card.label || 'NFC card'} · {card.student_name} · {card.grade} • Section {card.section}</small></div><div className="attendance-card-actions"><span className={`status ${card.active ? 'present' : 'pending'}`}>{card.active ? 'Demo/Active' : 'Inactive'}</span><button className="outline-btn" disabled={scanning || !card.active} onClick={() => void demoScan(card.uid)}><Icon>nfc</Icon>Scan</button></div></div>)}</Card></AdminLayout>;
}
